...
struct Info {
  ...
  TSet fst;
  int prec;
  enum { ANone, ALeft, ARight, ANonassoc } assoc;
  char name[IdntSz];
  char type[IdntSz];
};
...
void tszero(TSet *ts) { memset(ts, 0, sizeof *ts); }
...
int tcmpv(const void *a, const void *b) { return tcmp((Term *)a, (Term *)b); }
...
int resolve(Rule *r, Sym s, int st) {
  if (!r->prec || !is[s].prec) {
  conflict:
    if (fgrm)
  ...
  if (r->prec == is[s].prec) {
    if (is[s].assoc == ANone)
      goto conflict;
    return is[s].assoc;
  } else if (r->prec < is[s].prec)
    return ARight;
  else
    return ALeft;
}

void tblset(int *tbl, Item *i, Term *t) {
  int act;
  Sym s;
...
void setdef(Row *r, int w, int top) {
...
void tblout() {
  ...
  aout("yytrns", o, m);
  if (fhdr) {
    fputs("int yyparse(void* lexer, void** data);\n", fhdr);
...
int istok(int c) { return isalnum(c) || c == '_' || c == '%'; }
  ...
  for (;;) {
    ...
    do {
      ...
      while ((tk = nexttk()) == TIdnt || tk == TTokchr || tk == TPrec) {
        if (tk == TPrec) {
          tk = nexttk();
          if (tk != TIdnt || (s = findsy(idnt, 0)) >= ntk)
